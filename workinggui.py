...
root.geometry("400x600")  # Increased window size to accommodate the widgets
...
# Function to send 'SET' command and wait for confirmation from Arduino
def send_set_command():
    global confirmed_to_set
    ser.write(b'2')  # Send 'SET' command (enum value 2) to Arduino
    time.sleep(0.5)
...
set_end_button_1.grid(row=5, column=0, columnspan=2, pady=5)

# End X Position input
tk.Label(frame, text="End X Angle (Motor 1):", bg="#001F3F", fg="white").grid(row=6, column=0, pady=5, sticky='e')
end_x_entry = tk.Entry(frame, state='disabled')
end_x_entry.grid(row=6, column=1, pady=5)

# End Y Position input
tk.Label(frame, text="End Y Angle (Motor 2):", bg="#001F3F", fg="white").grid(row=7, column=0, pady=5, sticky='e')
end_y_entry = tk.Entry(frame, state='disabled')
end_y_entry.grid(row=7, column=1, pady=5)

# Button to set end position
set_end_button_2 = tk.Button(frame, text="Set End Position", command=set_end_positions, bg="#000000", fg="white")
set_end_button_2.grid(row=8, column=0, columnspan=2, pady=5)
...
set_time_button_2.grid(row=9, column=0, columnspan=2, pady=5)

# Time Duration input
tk.Label(frame, text="Time Duration (s):", bg="#001F3F", fg="white").grid(row=10, column=0, pady=5, sticky='e')
time_entry = tk.Entry(frame, state='disabled')
time_entry.grid(row=10, column=1, pady=5)
...
set_time_button.grid(row=11, column=0, columnspan=2, pady=5)

# Button to start tracking
start_tracking_button = tk.Button(frame, text="Start Tracking", command=start_tracking, bg="#000000", fg="white", state='disabled')
start_tracking_button.grid(row=12, column=0, columnspan=2, pady=5)
